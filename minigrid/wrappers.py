...
class OneHotPartialObsWrapper(gym.core.ObservationWrapper):
    ...
    def observation(self, obs):
        img = obs['image']
        out = np.zeros(self.observation_space.spaces['image'].shape,
                       dtype='uint8')
...
class RGBImgPartialObsWrapper(gym.core.ObservationWrapper):
    """
    Wrapper to use partially observable RGB image as only observation output
...
class FlatObsWrapper(gym.core.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        if mission != self.cachedStr:
            assert len(mission) <= self.maxStrLen, \
...
class DirectionObsWrapper(gym.core.ObservationWrapper):
    ...
    def reset(self):
        ...
        if not self.goal_position:
            self.goal_position = [x for x, y in enumerate(self.grid.grid)
                                  if isinstance(y, (Goal))]
            # when multiple goals, needs to be handled for other env types
            if len(self.goal_position) >= 1:
                self.goal_position = (int(self.goal_position[0] / self.height),
                                      self.goal_position[0] % self.width)
        return obs

    def observation(self, obs):
        slope = np.divide(self.goal_position[1] - self.agent_pos[1],
                          self.goal_position[0] - self.agent_pos[0])
        obs['goal_direction'] = \
            np.arctan(slope) if self.type == 'angle' else slope
        return obs
